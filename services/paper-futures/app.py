...
EXPOSURE = Gauge("paper_futures_gross_notional", "Current mark-to-market gross notional", ["strategy"])
ENTRY_EXPOSURE = Gauge("paper_futures_entry_notional", "Gross notional fixed at entry fills", ["strategy"])
RISK_USAGE = Gauge("paper_futures_risk_utilization_fraction", "Entry notional divided by configured maximum", ["strategy"])
...
def execute():
    try:
        ...
        for account in result["accounts"]:
            ...
            ENTRY_EXPOSURE.labels(name).set(account["entry_notional"])
            RISK_USAGE.labels(name).set(account["entry_notional"] / config.max_gross_notional)
